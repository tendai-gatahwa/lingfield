        ...
        try {
            ...
            foreach ($request->promote as $student_id => $student_data) {
                ...
                if (!$exit_record) {
                    ...
                    $data['class_id'] = gv($student_data, 'class');
                    $data['section_id'] = gv($student_data, 'section');
                    ...
                    $records = $this->studentRecordInfo(gv($student_data, 'class'), gv($student_data, 'section'))->pluck('studentDetail.user_id');
                    $this->sent_notifications('Student_Promote', $records, $data, ['Student', 'Parent']);
