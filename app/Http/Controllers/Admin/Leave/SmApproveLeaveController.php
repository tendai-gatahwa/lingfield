        ...
        try {
            ...
            try {
                ...
                if ($leave_request_data->role_id == 2) {
                    $data['class_id'] = $leave_request_data->student->studentRecord->class_id;
                    $data['section_id'] = $leave_request_data->student->studentRecord->section_id;
                    $records = $this->studentRecordInfo($data['class_id'], $data['section_id'])->pluck('studentDetail.user_id');
                    $this->sent_notifications($status, $records, $data, ['Student', 'Parent']);
                }
                ...
                $data['apply_date'] = $leave_request_data->apply_date;
                $data['leave_from'] = $leave_request_data->leave_from;
                $data['leave_to'] = $leave_request_data->leave_to;
                $data['reason'] = $leave_request_data->reason;
                if ($request->approve_status == 'A') {
                ...
                send_mail($user->email, $user->full_name, "leave_notification", $data);
            } catch (\Exception $e) {
